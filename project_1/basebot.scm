...
;; x(t) = v cos alpha t
;; assuming it starts is to simulate at the origin
...
(define root1
  (lambda (a b c)
    (/ (+ (- b) (determinant a b c)) (* a 2)
    ))
  )

(define (determinant a b c)
 (let ((val (- (square b) (* 4 a c))))
    (if (>= val 0)
     (sqrt val)
...
(define time-to-impact
  (lambda (vertical-velocity elevation)
    (max (root1 (- 4.9) vertical-velocity elevation)
    (root2 (- 4.9) vertical-velocity elevation)))
)
...
(define find-best-angle
  (lambda (velocity elevation)
    (define (best-angle-iter angle best-guess best-angle)
      (let ((guess (travel-distance-simple elevation velocity angle)))
        (if (<= angle 0)
          best-angle
          (if (> guess best-guess)
            (best-angle-iter (- angle alpha-increment) guess angle)
            (best-angle-iter (- angle alpha-increment) best-guess best-angle)))))
  (best-angle-iter 90 0 0)))
  ...
  (lambda (x0 y0 u0 v0 dt g m beta)
    (if (<= y0 0.0)
      ...
      (integrate (+ x0 (* u0 dt))
                 (+ y0 (* v0 dt))
                 (+ u0
                  (/ (* (- 1.0) beta u0 dt (sqrt (+ (* u0 u0) (* v0 v0))))
                    m))
                 (+ v0
                  (* (- dt)
                     (+ (/ (* beta
...
(define alpha-increment 0.05)
(define delta 0.25)
; We wish to apologize in advance to all mathematicians for the following:
(define inf 1e100)
(define dt 0.005)

(define find-fastest-angle
  (lambda (velocity elevation target)
    (define (best-angle-iter angle best-time best-angle)
      (let ((guess (travel-distance-with-time elevation velocity angle)))
        (if (<= angle (- 90))
          (list best-angle best-time)
          (if (and (< (cadr guess) (+ target delta))
                    (> (cadr guess) (- target delta))
                     (< (car guess) best-time ))
              (best-angle-iter (- angle alpha-increment) (car guess) angle)
          (best-angle-iter (- angle alpha-increment) best-time best-angle)))))
  (best-angle-iter 90 inf 0)))

(define travel-distance-with-time
  (lambda (elevation velocity angle)
   (integrate-with-time 0
              0
              elevation
              (* velocity (cos (degree2radian angle)))
              (* velocity (sin (degree2radian angle)))
...
(define integrate-with-time
  (lambda (time x0 y0 u0 v0 dt g m beta)
    (if (<= y0 0.0)
      (list time x0)
      (integrate-with-time (+ time dt)
                 (+ x0 (* u0 dt))
                 (+ y0 (* v0 dt))
                 (+ u0
                  (/ (* (- 1.0) beta u0 dt (sqrt (+ (* u0 u0) (* v0 v0))))
